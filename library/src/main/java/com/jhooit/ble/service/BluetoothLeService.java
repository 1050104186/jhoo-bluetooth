...
import com.jhooit.ble.model.BleSppGattAttributes;
...
public class BluetoothLeService extends Service {
    ...
    private int mConnectionState = BConstants.STATE_DISCONNECTED;
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                intentAction = BConstants.ACTION_GATT_CONNECTED;
                mConnectionState = BConstants.STATE_CONNECTED;
                ...
                intentAction = BConstants.ACTION_GATT_DISCONNECTED;
                mConnectionState = BConstants.STATE_DISCONNECTED;
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                if (mNotifyCharacteristic != null) {
                    broadcastUpdate(BConstants.ACTION_GATT_SERVICES_DISCOVERED);
                    //使能Notify
                ...
                if (service == null) {
                    ...
                    broadcastUpdate(BConstants.ACTION_GATT_SERVICES_NO_DISCOVERED);
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                broadcastUpdate(BConstants.ACTION_WRITE_SUCCESSFUL);
                Log.v(TAG, "Write OK");
            }
        }
    };
    ...
    private void broadcastUpdate(final String action, final BluetoothGattCharacteristic characteristic) {
        ...
        if (UUID_BLE_SPP_NOTIFY.equals(characteristic.getUuid())) {
            ...
            if (data != null && data.length > 0) {
                intent.putExtra(BConstants.EXTRA_DATA, data);
            }
        }
        sendBroadcast(intent);
    }
    ...
    public boolean connect(final String address) {
        ...
        if (mBluetoothDeviceAddress != null && address.equals(mBluetoothDeviceAddress) && mBluetoothGatt != null) {
            ...
            if (mBluetoothGatt.connect()) {
                mConnectionState = BConstants.STATE_CONNECTING;
                return true;
            } else {
                return false;
            }
    ...
    public void setCharacteristicNotification(BluetoothGattCharacteristic characteristic, boolean enabled) {
        ...
        if (UUID_BLE_SPP_NOTIFY.equals(characteristic.getUuid())) {
            UUID uuid = UUID.fromString(BleSppGattAttributes.CLIENT_CHARACTERISTIC_CONFIG);
